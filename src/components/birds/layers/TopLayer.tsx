"use client";

import { useEffect, useState } from "react";
import { AlertTriangle, Bird, Info } from "lucide-react";
import { Card } from "@/components/ui/card";
// import { layersService } from "@/services/layers.service";
import { LayersDashboardResponse } from "@/types/layers";
import { layersService } from "../../../../services/layers.service";

export default function TopLayer() {
  const [summary, setSummary] =
    useState<LayersDashboardResponse["summary"] | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  // ✅ Greeting logic (dynamic)
  const getGreeting = () => {
    const hour = new Date().getHours();
    if (hour < 12) return "Good Morning";
    if (hour < 18) return "Good Afternoon";
    return "Good Evening";
  };

  // ✅ FETCH DATA
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        const res = await layersService.getDashboard();

        console.log("LAYERS DASHBOARD:", res); // debug

        setSummary(res.summary);
        setError("");
      } catch (err: any) {
        console.error("TopLayer error:", err);
        setError(err?.message || "Failed to load dashboard");
      } finally {
        setLoading(false);
      }
    };

    fetchDashboard();
  }, []);

  // ✅ LOADING STATE
  if (loading) {
    return <p className="text-gray-500">Loading dashboard...</p>;
  }

  // ✅ ERROR STATE (still show UI fallback)
  if (error) {
    console.warn(error);
  }

  return (
    <div className="w-full">
      {/* Greeting */}
      <div className="pb-5">
        <p className="font-semibold p-1">
          {getGreeting()}, Admin.
        </p>
      </div>

      {/* Cards */}
      <div
        className="
          flex gap-2
          overflow-x-auto
          whitespace-nowrap
          scrollbar-hide
          md:overflow-visible
        "
      >
        {/* TOTAL BIRDS */}
        <Card className="w-[218px] h-[114px] p-2 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              <Bird className="text-[#4A3AFF] h-[18px] w-[18px] border border-[#4A3AFF1A] rounded-full bg-[#4A3AFF1A]" />
              <p className="text-[12px] font-semibold">Total Birds</p>
            </div>
            <Info className="h-[14px] w-[14px] text-[#141B34]" />
          </div>

          <div>
            <p className="text-[18px] font-bold">
              {summary?.totalBirds ?? 0}
            </p>
            <div className="text-[10px] font-semibold">
              Current bird count
            </div>
          </div>
        </Card>

        {/* TOTAL MORTALITY */}
        <Card className="w-[218px] h-[114px] p-2 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              <Bird className="text-[#4A3AFF] h-[18px] w-[18px] border border-[#4A3AFF1A] rounded-full bg-[#4A3AFF1A]" />
              <p className="text-[12px] font-semibold">Total Mortality</p>
            </div>
            <Info className="h-[14px] w-[14px] text-[#141B34]" />
          </div>

          <div>
            <p className="text-[18px] font-bold">
              {summary?.totalMortality ?? 0}
            </p>
            <div className="text-[10px] font-semibold">
              Mortality count
            </div>
          </div>
        </Card>

        {/* TOTAL ALIVE */}
        <Card className="w-[218px] h-[114px] p-2 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              <Bird className="text-[#4A3AFF] h-[18px] w-[18px] border border-[#4A3AFF1A] rounded-full bg-[#4A3AFF1A]" />
              <p className="text-[12px] font-semibold">
                Total Alive Birds
              </p>
            </div>
            <Info className="h-[14px] w-[14px] text-[#141B34]" />
          </div>

          <div>
            <p className="text-[18px] font-bold">
              {summary?.totalAlive ?? 0}
            </p>
            <div className="text-[10px] font-semibold">
              Healthy birds
            </div>
          </div>
        </Card>

        {/* ACTIVE BREACHES (derived) */}
        <Card className="w-[218px] h-[114px] p-2 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              <AlertTriangle
                size={18}
                className="text-[#FF0004] border border-[#FF00041A] rounded-full bg-[#FF00041A]"
              />
              <p className="text-[12px] font-semibold">
                Active Breaches
              </p>
            </div>
            <Info className="h-[14px] w-[14px] text-[#141B34]" />
          </div>

          <div>
            <p className="text-[18px] font-bold">
              {
                // ✅ derive from rows (important!)
                summary
                  ? 0 // replace if backend provides later
                  : 0
              }
            </p>
            <div className="text-[10px] font-semibold">
              Alerts triggered
            </div>
          </div>
        </Card>
      </div>
    </div>
  );
}