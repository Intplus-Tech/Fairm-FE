...
export default function AddPenForm({
  ...
  const [houseType, setHouseType] = useState<HouseType>("deep_litter");
  const [feeder, setFeeder] = useState<FeederType>("auto_feeders");
  const [sensors, setSensors] = useState<SensorType[]>([]);
  const [birdType, setBirdType] = useState<BirdType>("broiler");
  const [breed, setBreed] = useState<BreedType>("cobb_500");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Pen created successfully. Proceeding to alert configuration...", {
        duration: 4000,
      });
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide pr-1">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-6">
          ...
          <div>
            ...
            <select
              value={feeder}
              onChange={(e) => setFeeder(e.target.value as FeederType)}
              className="w-full mt-1 rounded-lg border px-3 py-2 text-sm"
            >
          ...
          <div>
            ...
            <input
              type="number"
              value={noOfBirds}
              onChange={(e) => setNoOfBirds(Number(e.target.value))}
              className="w-full mt-1 rounded-lg border px-3 py-2 text-sm"
            />
          </div>
          ...
          <div>
            ...
            <input
              type="number"
              value={ageInWeeks}
              onChange={(e) => setAgeInWeeks(Number(e.target.value))}
              className="w-full mt-1 rounded-lg border px-3 py-2 text-sm"
            />
          </div>
